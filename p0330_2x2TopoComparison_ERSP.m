...
tdcThetaOnset = squeeze(mean(mean(erspTDC(:, thetaIdx, onsetIdx, :),2),3));

fxs40Hz = squeeze(mean(mean(erspFXS(:, gammaIdx, assrIdx, :),2),3));
tdc40Hz = squeeze(mean(mean(erspTDC(:, gammaIdx, assrIdx, :),2),3));

fxsThetaOffset = squeeze(mean(mean(erspFXS(:, thetaIdx, offsetIdx, :),2),3));
tdcThetaOffset = squeeze(mean(mean(erspTDC(:, thetaIdx, offsetIdx, :),2),3));
...
subplot(3,3,4)
topoplot(mean(tdcThetaOnset,2), EEG.chanlocs, 'maplimits', [-1 1])
title('TDC Onset Theta')

subplot(3,3,5)
topoplot(mean(tdc40Hz,2), EEG.chanlocs, 'maplimits', [-1 1])
title('TDC 40Hz ASSR')

subplot(3,3,6)
topoplot(mean(tdcThetaOffset,2), EEG.chanlocs, 'maplimits', [-1 1])
...
exportgraphics(gcf, '/srv/Makoto/ASSR/p0330_2x2TopoComparison_ERSP/ERSP.pdf', 'ContentType', 'vector')
% print('/srv/Makoto/ASSR/p0330_2x2TopoComparison_ERSP/ERSP', '-r200', '-djpeg95')
% print('/srv/Makoto/ASSR/p0330_2x2TopoComparison_ERSP/ERSP', '-r200', '-dsvg')