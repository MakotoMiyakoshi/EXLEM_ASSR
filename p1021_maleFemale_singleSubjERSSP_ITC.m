...
% 
% % FXS_M.
% pTensor     = single(zeros(size(itcFXS_M,1), size(itcFXS_M,2), size(itcFXS_M,3)));
% ersspTensor = single(zeros(size(itcFXS_M,2), size(itcFXS_M,3), size(itcFXS_M,4)));
% for subjIdx = 1:size(itcFXS_M, 4)
%     currentData = itcFXS_M(:,:,:,subjIdx);
...
% end
% save('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_FXS_M', 'ersspTensor', 'timeBins', 'wtFreqBins', '-nocompression')
% 
% % FXS_F.
% pTensor     = single(zeros(size(itcFXS_F,1), size(itcFXS_F,2), size(itcFXS_F,3)));
% ersspTensor = single(zeros(size(itcFXS_F,2), size(itcFXS_F,3), size(itcFXS_F,4)));
% for subjIdx = 1:size(itcFXS_F, 4)
%     currentData = itcFXS_F(:,:,:,subjIdx);
%     for chIdx = 1:size(currentData, 1)
%         for freqIdxIdx = 1:size(currentData,2)
%             currentFreqP = squeeze(normcdf(currentData(chIdx,freqIdxIdx,:), squeeze(mean(currentData(chIdx,freqIdxIdx,baselineIdx),3)), squeeze(std(currentData(chIdx,freqIdxIdx,baselineIdx),0,3))));
%             pTensor(chIdx,freqIdxIdx,:) = currentFreqP;
...
%     ersspTensor(:,:,subjIdx) = pTensorSumMasks;
% end
% save('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_FXS_F', 'ersspTensor', 'timeBins', 'wtFreqBins', '-nocompression')
% 
% % HCS_M.
% pTensor     = single(zeros(size(itcHCS_M,1), size(itcHCS_M,2), size(itcHCS_M,3)));
% ersspTensor = single(zeros(size(itcHCS_M,2), size(itcHCS_M,3), size(itcHCS_M,4)));
% for subjIdx = 1:size(itcHCS_M, 4)
%     currentData = itcHCS_M(:,:,:,subjIdx);
%     for chIdx = 1:size(currentData, 1)
%         for freqIdxIdx = 1:size(currentData,2)
%             currentFreqP = squeeze(normcdf(currentData(chIdx,freqIdxIdx,:), squeeze(mean(currentData(chIdx,freqIdxIdx,baselineIdx),3)), squeeze(std(currentData(chIdx,freqIdxIdx,baselineIdx),0,3))));
%             pTensor(chIdx,freqIdxIdx,:) = currentFreqP;
...
%     ersspTensor(:,:,subjIdx) = pTensorSumMasks;
% end
% save('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_HCS_M', 'ersspTensor', 'timeBins', 'wtFreqBins', '-nocompression')
% 
% % HCS_F.
% pTensor     = single(zeros(size(itcHCS_F,1), size(itcHCS_F,2), size(itcHCS_F,3)));
% ersspTensor = single(zeros(size(itcHCS_F,2), size(itcHCS_F,3), size(itcHCS_F,4)));
% for subjIdx = 1:size(itcHCS_F, 4)
%     currentData = itcHCS_F(:,:,:,subjIdx);
%     for chIdx = 1:size(currentData, 1)
%         for freqIdxIdx = 1:size(currentData,2)
%             currentFreqP = squeeze(normcdf(currentData(chIdx,freqIdxIdx,:), squeeze(mean(currentData(chIdx,freqIdxIdx,baselineIdx),3)), squeeze(std(currentData(chIdx,freqIdxIdx,baselineIdx),0,3))));
%             pTensor(chIdx,freqIdxIdx,:) = currentFreqP;
...
%     ersspTensor(:,:,subjIdx) = pTensorSumMasks;
% end
% save('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_HCS_F', 'ersspTensor', 'timeBins', 'wtFreqBins', '-nocompression')


%%
FXS_M = load('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_FXS_M.mat');
FXS_F = load('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_FXS_F.mat');
HCS_M = load('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_HCS_M.mat');
HCS_F = load('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_itc_HCS_F.mat');
FXS_M = FXS_M.ersspTensor;
FXS_F = FXS_F.ersspTensor;
HCS_M = HCS_M.ersspTensor;
HCS_F = HCS_F.ersspTensor;

% clear itcFXS_* itcHCS_* stackData
% save dumpedFiles
...
gmFXS_F_onsetMask  = L==4;
gmFXS_F_offsetMask = L==48;
...
%     sumPixelVec(maskIdx) = sumPixels;
% end
% gmFXS_MF_onsetMask  = L==186;
% gmFXS_MF_offsetMask = L==790;
...
gmHCS_M_onsetMask  = L==1;
gmHCS_M_offsetMask = L==10;
...
gmHCS_F_onsetMask  = L==1;
gmHCS_F_offsetMask = L==6; 
...
%     sumPixelVec(maskIdx) = sumPixels;
% end
% gmHCS_MF_onsetMask  = L==9;
% gmHCS_MF_offsetMask = L==262;
...
imagesc(timeBins, 1:100, mean(FXS_M,3), [0 90])
hold on
...
imagesc(timeBins, 1:100, mean(FXS_F,3), [0 90])
hold on
...
imagesc(timeBins, 1:100, mean(HCS_M,3), [0 90])
hold on
...
imagesc(timeBins, 1:100, mean(HCS_F,3), [0 90])
hold on
...
% stats1 = {[0.0183]}    {[3.5752]}    {[7.3830]}
% df1    = {[1 88]}    {[1 88]}    {[1 88]}
% pvals1 = {[0.8928]}    {[0.0619]}    {[0.0079]}
...
% stats2 = {[1.3472]}    {[21.7502]}    {[0.3930]}
% df2    = {[1 88]}    {[1 88]}    {[1 88]}
% pvals2 = {[0.2489]}    {[1.1027e-05]}    {[0.5323]}
...
% stats3 = {[0.0737]}    {[0.0097]}    {[1.8716]}
% df3    = {[1 88]}    {[1 88]}    {[1 88]}
% pvals3 = {[0.7866]}    {[0.9219]}    {[0.1748]}

print('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_ITC_maleFemale', '-dsvg')
print('/srv/Makoto/ASSR/p1020_maleFemale_singleSubjERSSP_ITC/ERSSP_ITC_maleFemale', '-djpeg95', '-r200')