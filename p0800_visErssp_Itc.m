...
% FXS.
pTensor = zeros(size(itcFXS));
for chIdx = 1:size(itcFXS, 1)
    for freqIdxIdx = 1:size(itcFXS,2)
        currentFreqP = squeeze(normcdf(itcFXS(chIdx,freqIdxIdx,:), squeeze(mean(itcFXS(chIdx,freqIdxIdx,baselineIdx),3)), squeeze(std(itcFXS(chIdx,freqIdxIdx,baselineIdx),0,3))));
...
save('/srv/Makoto/ASSR/p0800_visErssp_Itc/itc_pTensor_FXS', 'pTensor_rTale', 'timeBins', 'wtFreqBins', '-nocompression')

% HCS.
pTensor = zeros(size(itcHCS));
for chIdx = 1:size(itcHCS, 1)
    for freqIdxIdx = 1:size(itcFXS,2)
        currentFreqP = squeeze(normcdf(itcHCS(chIdx,freqIdxIdx,:), squeeze(mean(itcHCS(chIdx,freqIdxIdx,baselineIdx),3)), squeeze(std(itcHCS(chIdx,freqIdxIdx,baselineIdx),0,3))));
        pTensor(chIdx,freqIdxIdx,:) = currentFreqP;
    end
end
...
% Dump pTensor_rTale.
pTensor_rTale = single(pTensor_rTale);
save('/srv/Makoto/ASSR/p0800_visErssp_Itc/itc_pTensor_HCS', 'pTensor_rTale', 'timeBins', 'wtFreqBins', '-nocompression')
...
print('/srv/Makoto/ASSR/p0800_visErssp_Itc/itcSigMaskSum_groupComparison', '-dsvg')
print('/srv/Makoto/ASSR/p0800_visErssp_Itc/itcSigMaskSum_groupComparison', '-djpeg95', '-r200')
...
print('/srv/Makoto/ASSR/p0800_visErssp_Itc/HcsFCzERSP', '-dsvg')
print('/srv/Makoto/ASSR/p0800_visErssp_Itc/HcsFCzERSP', '-djpeg95', '-r200')


%% Ordinary two-sample t-test on 40-Hz ASSR at FCz.
fdxGamma = squeeze(mean(stackData(6,80,assrIdx,fxsIdx),3));
tdcGamma = squeeze(mean(stackData(6,80,assrIdx,hcsIdx),3));

[H,P,CI,STATS] = ttest2(fdxGamma, tdcGamma);
disp(sprintf('t(%d)=%.1f, p=%.4f', STATS.df, STATS.tstat, P)) % t(66)=-1.6, p=0.1072
...
fxsTheta1 = squeeze(mean(mean(stackData(6,thetaIdx,onsetIdx, fxsIdx),2),3));
hcsTheta1 = squeeze(mean(mean(stackData(6,thetaIdx,onsetIdx, hcsIdx),2),3));

[H,P,CI,STATS] = ttest2(fxsTheta1, hcsTheta1);
disp(sprintf('t(%d)=%.1f, p=%.4f', STATS.df, STATS.tstat, P)) % t(66)=0.3, p=0.7285
...
fxsTheta2 = squeeze(mean(mean(stackData(6,thetaIdx,offsetIdx, fxsIdx),2),3));
hcsTheta2 = squeeze(mean(mean(stackData(6,thetaIdx,offsetIdx, hcsIdx),2),3));

[H,P,CI,STATS] = ttest2(fxsTheta2, hcsTheta2);
disp(sprintf('t(%d)=%.1f, p=%.4f', STATS.df, STATS.tstat, P)) % t(66)=0.6, p=0.5598
