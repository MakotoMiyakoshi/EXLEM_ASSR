...
for setIdx = 1:length(allSets)
    ...
    %%% Save data. %%%
    %%%%%%%%%%%%%%%%%%
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_elecErspMean',   dataName), 'elecErspMean',   '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_elecErspMedian', dataName), 'elecErspMedian', '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_elecItcMean',    dataName), 'elecItcMean',    '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_elecItcMedian',  dataName), 'elecItcMedian',  '-nocompression')

    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_icErspMean',   dataName), 'icErspMean',   '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_icErspMedian', dataName), 'icErspMedian', '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_icItcMean',    dataName), 'icItcMean',    '-nocompression')
    save(sprintf('/srv/Makoto/ASSR/p0420_epochBrain/%s_icItcMedian',  dataName), 'icItcMedian',  '-nocompression')
end