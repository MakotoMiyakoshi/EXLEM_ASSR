...
CzERSP_sigMask_FXS = squeeze(ersp_pTensor_FXS(elecIdx,:,:))<0.001;
    %{
    figure
    imagesc(CzERSP_sigMask_FXS)
    %}
CzERSP_sigMask_FXS(:, [1:3800 4800:end]) = 0;

% Define functional ROI for HCS ERSP at '40Hz ROI' (i.e. 'Lemniscal ROI').
CzERSP_sigMask_HCS = squeeze(ersp_pTensor_HCS(elecIdx,:,:))<0.001;
    %{
    figure
    imagesc(CzERSP_sigMask_HCS)
    %}
CzERSP_sigMask_HCS(:,[1:4000 4500:5000]) = 0;
CzERSP_sigMask_HCS(42:end,            :) = 0;
...
xlim([0.5 2.5])
ylim([0 3])
title(sprintf('Offset-evoked theta, R^2=%.3f, t(%d)=%.2f, p=%.4f', lmeModel1.Rsquared.Adjusted, lmeModel1.Coefficients.DF(2), lmeModel1.Coefficients.tStat(2), lmeModel1.Coefficients.pValue(2)))

sgtitle('Grand-mean ERSP at Cz')

print('/srv/Makoto/ASSR/p0740_visErspItc_Cz_Offset/ERSP_offsetTheta', '-djpeg', '-r200')
print('/srv/Makoto/ASSR/p0740_visErspItc_Cz_Offset/ERSP_offsetTheta', '-dsvg', '-r200')
