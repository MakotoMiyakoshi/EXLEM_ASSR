...
% Scalp topos.
onsetFXS_M_topo = squeeze(mean(mean(mean(itcFXS_M(:, deltaThetaAlphaIdx, onsetIdx, :),2),3),4));
onsetFXS_F_topo = squeeze(mean(mean(mean(itcFXS_F(:, deltaThetaAlphaIdx, onsetIdx, :),2),3),4));
onsetHCS_M_topo = squeeze(mean(mean(mean(itcHCS_M(:, deltaThetaAlphaIdx, onsetIdx, :),2),3),4));
onsetHCS_F_topo = squeeze(mean(mean(mean(itcHCS_F(:, deltaThetaAlphaIdx, onsetIdx, :),2),3),4));
assrFXS_M_topo = squeeze(mean(mean(mean(itcFXS_M(:, gammaIdx, assrIdx, :),2),3),4));
assrFXS_F_topo = squeeze(mean(mean(mean(itcFXS_F(:, gammaIdx, assrIdx, :),2),3),4));
assrHCS_M_topo = squeeze(mean(mean(mean(itcHCS_M(:, gammaIdx, assrIdx, :),2),3),4));
assrHCS_F_topo = squeeze(mean(mean(mean(itcHCS_F(:, gammaIdx, assrIdx, :),2),3),4));
offsetFXS_M_topo = squeeze(mean(mean(mean(itcFXS_M(:, deltaThetaAlphaIdx, offsetIdx, :),2),3),4));
offsetFXS_F_topo = squeeze(mean(mean(mean(itcFXS_F(:, deltaThetaAlphaIdx, offsetIdx, :),2),3),4));
offsetHCS_M_topo = squeeze(mean(mean(mean(itcHCS_M(:, deltaThetaAlphaIdx, offsetIdx, :),2),3),4));
offsetHCS_F_topo = squeeze(mean(mean(mean(itcHCS_F(:, deltaThetaAlphaIdx, offsetIdx, :),2),3),4));

% ROI mean.
onsetFXS_M_roiMean = squeeze(mean(mean(itcFXS_M(55, deltaThetaAlphaIdx, onsetIdx, :),2),3));
onsetFXS_F_roiMean = squeeze(mean(mean(itcFXS_F(55, deltaThetaAlphaIdx, onsetIdx, :),2),3));
onsetHCS_M_roiMean = squeeze(mean(mean(itcHCS_M(55, deltaThetaAlphaIdx, onsetIdx, :),2),3));
onsetHCS_F_roiMean = squeeze(mean(mean(itcHCS_F(55, deltaThetaAlphaIdx, onsetIdx, :),2),3));
assrFXS_M_roiMean = squeeze(mean(mean(itcFXS_M(6, gammaIdx, assrIdx, :),2),3));
assrFXS_F_roiMean = squeeze(mean(mean(itcFXS_F(6, gammaIdx, assrIdx, :),2),3));
assrHCS_M_roiMean = squeeze(mean(mean(itcHCS_M(6, gammaIdx, assrIdx, :),2),3));
assrHCS_F_roiMean = squeeze(mean(mean(itcHCS_F(6, gammaIdx, assrIdx, :),2),3));
offsetFXS_M_roiMean = squeeze(mean(mean(itcFXS_M(55, deltaThetaAlphaIdx, offsetIdx, :),2),3));
offsetFXS_F_roiMean = squeeze(mean(mean(itcFXS_F(55, deltaThetaAlphaIdx, offsetIdx, :),2),3));
offsetHCS_M_roiMean = squeeze(mean(mean(itcHCS_M(55, deltaThetaAlphaIdx, offsetIdx, :),2),3));
offsetHCS_F_roiMean = squeeze(mean(mean(itcHCS_F(55, deltaThetaAlphaIdx, offsetIdx, :),2),3));
...
imagesc(timeBins, 1:100, fxsmGm_Cz, [0 0.6])
...
imagesc(timeBins, 1:100, fxsfGm_Cz, [0 0.6])
set(gca, 'ytick', freqIdx, 'yticklabel', freqLabels)
axis xy
...
imagesc(timeBins, 1:100, hcsmGm_Cz, [0 0.6])
set(gca, 'ytick', freqIdx, 'yticklabel', freqLabels)
axis xy
...
imagesc(timeBins, 1:100, hcsfGm_Cz, [0 0.6])
set(gca, 'ytick', freqIdx, 'yticklabel', freqLabels)
axis xy
...
topoplot(onsetFXS_M_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 2)
topoplot(onsetFXS_F_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 3)
topoplot(assrFXS_M_topo, EEG.chanlocs, 'maplimits', [0 0.6]);
nexttile(nestedLayout3, 4)
topoplot(assrFXS_F_topo, EEG.chanlocs, 'maplimits', [0 0.6]);
nexttile(nestedLayout3, 5)
topoplot(offsetFXS_M_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 6)
topoplot(offsetFXS_F_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 7)
topoplot(onsetHCS_M_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 8)
topoplot(onsetHCS_F_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 9)
topoplot(assrHCS_M_topo, EEG.chanlocs, 'maplimits', [0 0.6]);
nexttile(nestedLayout3, 10)
topoplot(assrHCS_F_topo, EEG.chanlocs, 'maplimits', [0 0.6]);
nexttile(nestedLayout3, 11)
topoplot(offsetHCS_M_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
nexttile(nestedLayout3, 12)
topoplot(offsetHCS_F_topo, EEG.chanlocs, 'maplimits', [0 0.4]);
...
[tStats_onset, df_onset, pvals_onset] = statcond({onsetFXS_M_roiMean' onsetFXS_F_roiMean'; onsetHCS_M_roiMean' onsetHCS_F_roiMean'});
    % t:{[0.0996]}    {[0.8473]}    {[0.1124]}
    % df: {[1 88]}    {[1 88]}    {[1 88]}
    % p:  {[0.7530]}    {[0.3598]}    {[0.7382]}
...
[tStats_assr, df_assr, pvals_assr] = statcond({assrFXS_M_roiMean' assrFXS_F_roiMean'; assrHCS_M_roiMean' assrHCS_F_roiMean'});
    % t: {[5.0969e-05]}    {[2.7996]}    {[2.8308]}
    % df: {[1 88]}    {[1 88]}    {[1 88]}
    % p: {[0.9943]}    {[0.0978]}    {[0.0960]}
...
[tStats_offset, df_offset, pvals_offset] = statcond({offsetFXS_M_roiMean' offsetFXS_F_roiMean'; offsetHCS_M_roiMean' offsetHCS_F_roiMean'});
    % t: {[1.4024]}    {[6.0523]}    {[6.8679]}
    % df: {[1 88]}    {[1 88]}    {[1 88]}
    % p:  {[0.2395]}    {[0.0158]}    {[0.0103]}
...
disp(sprintf('%.3f %.3f %.3f',offsetD_MaleFemale, offsetD_FxsHcs, offsetD_F2))
    % Cohen 0.231 0.289 0.078

exportgraphics(gcf, '/srv/Makoto/ASSR/p1310_itcMaleFemale/ITCs.pdf', 'ContentType', 'vector', 'Resolution', 300, 'Colorspace', 'rgb')