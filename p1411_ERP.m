...
mainLayout = tiledlayout(5,3,"TileSpacing", "tight", "Padding", "tight");
...
disp(sprintf('t(%d)=%.3f, p=%.5f, Cohen''s d = %.3f.', STATS3.df, STATS3.tstat, P3, d3))
    % t(60)=4.937, p=0.000, Cohen's d = 1.257.

%% Inset plot for PSD.
fxsAssrErp = squeeze(fxsERP(55,1501:4000,:));
hcsAssrErp = squeeze(hcsERP(55,1501:4000,:));
...
[H8,P8,CI8,STATS8] = ttest2(fxs40HzPower, hcs40HzPower);
d8 = cohensD(fxs40HzPower, hcs40HzPower);
...
[H9,P9,CI9,STATS9] = ttest2(fxsSlopePower, hcsSlopePower);
d9 = cohensD(fxsSlopePower, hcsSlopePower);

% Prepare an inset.
nexttile(mainLayout, 10)
subplotPosition = get(gca,'position');
insetRelativePosition = [0.55 0.55 0.4 0.4]; % NorthEast. 
...
[H4,P4,CI4,STATS4] = ttest2(squeeze(fxsERP(55,4096,:))', squeeze(hcsERP(55,4096,:))');
d4 = cohensD(squeeze(fxsERP(55,4096,:))', squeeze(hcsERP(55,4096,:))');
...
exportgraphics(gcf, '/srv/Makoto/ASSR/p1411_ERP/ERPs.pdf', 'ContentType', 'vector', 'Resolution', 300, 'Colorspace', 'rgb')
save('/srv/Makoto/ASSR/p1411_ERP/resultTableErp.mat', 'resultTableErp')